...
- 修改型增强表单禁止使用会重置控件的裸 `update()`；提交期间保持当前输入，成功响应必须携带服务端确认的变更实体或删除 ID，前端立即就地合并。统一使用 `update({ reset: false, invalidateAll: false })`，之后只按需定向失效根布局身份或提醒依赖；失败时保留输入并就地提示，不得恢复整页快照响应。
