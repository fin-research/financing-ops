...
- 项目删除在单一事务内锁定并核对目标，同步删除项目任务及其提醒记录；重复删除按幂等成功处理。项目页首屏只执行 1 次项目集合 SQL；新建/编辑弹窗真正打开时才通过 `/projects/options` 以 1 次 SQL 增量加载人员和启用 SOP。创建和修改成功后只查询并回传一个变更项目，浏览器基于本地项目源重新计算公共时间轴，不再回传完整项目列表。
