...
- 融资项目通过 `debt.project_id` 一对一绑定已有负债；项目建档只选择未绑定且存在启用 SOP 的负债，不创建新负债。删除项目会同步删除项目任务及其提醒记录，但保留负债并解除绑定。
...
- 项目删除在单一事务内锁定并核对目标，同步删除项目任务及其提醒记录，并将关联负债恢复为可再次建档状态；重复删除按幂等成功处理。项目页仍保持 3 次请求级集合 SQL，不增加 N+1 查询。
