...
- 每个受保护请求调用一次 Neon Auth 会话接口，并用一次 Hyperdrive 查询完成 `people.neon_auth_user_id` 与业务角色映射；该查询只返回身份与头像版本，不再传输 base64 头像。头像由私有缓存的 `/financing/avatar` 独立读取，未变更时不重复传输。
...
- 项目删除在单一事务内锁定并核对目标，同步删除项目任务及其提醒记录；重复删除按幂等成功处理。项目页加载仍保持 3 次请求级集合 SQL，不增加 N+1 查询；创建和修改成功后只查询并回传一个变更项目，浏览器基于本地项目源重新计算公共时间轴，不再回传完整项目列表。
...
- 人员、SOP 总览和个人资料的新增或修改成功响应只回传服务端确认的单个人员、SOP、规则或资料实体，前端立即合并并保留表单状态；随后仅按需同步顶栏身份或提醒，不重新请求当前页面。
- 提醒发送历史首批最多返回 50 条；“加载更多”使用 `(delivery_date, created_at, id)` 游标继续读取下一批 50 条，不重复统计或返回既有记录。
