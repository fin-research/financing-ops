...
- `/financing/projects` 可新建、修改和删除独立项目。新建时选择启用中的 SOP，并自行填写项目名称、融资主体、规模和计划日期。
- 项目名称、融资品种、主体、规模和计划日期均由项目独立维护；负责人可暂不分配，项目节点按所选启用 SOP 生成。
...
- SOP 节点保存与项目详情修改均保留当前表单状态，服务端成功响应直接回传最新实体并由前端就地合并；失败不清空输入。SOP 节点通过左侧手柄拖拽排序，右下角固定加号打开新增节点模态框。
