...
- 负债周报 `saveSnapshot` action 成功时返回快照 ID、报告日对应的成功文案和缺失模块清单；浏览器随后显式导航到当前报告日并强制重跑页面 load，只读取刚保存的索引与 R2 快照，不重新请求 Neon、Choice 或 DM。全局顶栏据此分别显示生成结果与待核对系统消息；若页面刷新失败，明确提示快照已保存并要求手动刷新。
- 进入负债周报或切换 `?date=yyyy-mm-dd` 时只按报告日读取已有快照；无快照时不发起数据拉取，但仍渲染完整周报模块、表头与图表空状态。点击生成后并行请求统一数据服务的 `/data/choice/ctr`、`/data/broker-bond-registrations`，以及 Neon Data API 的 `/rpc/liability_weekly_report_data` 和 `/liability_market_rate_observations`。浏览器用原始市场观测计算信用利差并合并载荷；市场利率视图不可用时只将市场利率与信用利差模块留空并写入待核对项，不阻断其他业务数据生成和快照保存。保存 action 只接收、白名单校验并固化结果，不在 financing Worker 内代理上游或执行报表聚合查询。
...
- `GET /financing/data/import/[id]`：管理员直接读取 Workflow 实例状态、最终勾稽结果和安全错误；前端仅在活跃状态轮询。不存在导入记录列表接口。
...
- 负债周报只允许固定调用 `financing.liability_weekly_report_data(date)`，并只读 `financing.liability_market_rate_observations` 的三列原始观测；PostgreSQL 业务 RPC 返回版本化 JSON，历史月末趋势读取冻结汇总，当前报告月份实时计算。浏览器按同指标日配对计算信用利差，服务端保存前再次校验报告日和字段边界。RPC 与视图以 `authenticated` 角色授权为边界，不额外检查 `people` 关联；可编辑数据表继续使用既有人员 RLS。

## 错误与状态码

- 输入错误使用 400；未登录 401；无权限 403；不存在 404；并发或唯一性冲突 409；Auth 暂不可用 503。
- 错误消息应可直接指导用户修正，不泄露 SQL、Secret、Cookie 或上游响应正文。
- 文件元数据、Protobuf/Brotli 编码、工作簿口径或 Workflow 事件大小错误返回 400；编码媒体类型错误返回 415；Workflow 绑定不可用返回 503。同一内容不会返回并发冲突，而是复用或重启同一实例。
