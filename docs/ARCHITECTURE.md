...
- `src/lib/server/liability-weekly-reports.js` 负责负债周报数据来源状态、显式手动生成和历史快照索引；Choice EDB/CTR 只在生成 action 内各请求一次。
...
- 负债周报生成不是页面 load 或 Cron 的副作用：用户确认点击后才读取 Choice，完整报告 JSON 写入现有 `eastmoney` R2 桶的 `debt-report/` 前缀，Neon `liability_weekly_report_runs` 保存基准日、来源清单、缺失模块和内容哈希；历史页只读 R2 快照。
