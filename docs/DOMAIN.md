...
- `financial_monthly_data` 宽表按月末保存财务历史，一月一行，基础金额按列维护，两项资产负债率为数据库计算列；`finance_parameters` 只保留迁移档案，业务取数使用 `finance_parameters_as_of(date)`。`debt_limit_configs` 保存负债品种额度和计算模式。
