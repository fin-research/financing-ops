...
- 内部测试阶段，SvelteKit 非安全方法对 `admin`、`handler`、`reviewer` 三种启用角色全部开放；身份有效性仍由 Neon Auth 会话和启用中的 `people` 记录共同校验。
- 三种角色均可维护人员主档、Neon Auth 登录、项目、SOP、提醒、周报快照和数据后台；这是一项明确的内部测试策略，不代表生产权限基线。
- 数据后台使用独立的 PostgreSQL RLS 规则：当前三种启用业务角色均可编辑白名单中的负债、监管参数和额度。
- 人员与账号管理必须防止停用或删除当前用户，并至少保留一个启用管理员。
- 本人任务状态专用 action 仍同时校验当前 `personId` 与任务 `assignee_id`，更新 SQL 再带相同负责人条件防止并发越权；完整任务维护在内部测试阶段向三种角色开放。
- 权限判断必须由服务端执行；隐藏或禁用按钮只用于正确表达界面能力，不是授权控制。
- 借入资金汇总表提交和进度接口在内部测试阶段向三种角色开放；POST 仍要求请求 `Origin` 与当前站点一致，未登录或无有效业务角色的请求不得读取或写入导入数据。
