...
-- Auth0 identity is validated by the Worker and scoped to a single SQL transaction.
...
SET search_path = pg_catalog, financing AS $$
  SELECT EXISTS (
    ...
    WHERE person.active = TRUE AND (
      (NULLIF(current_setting('request.financing.user_id', TRUE), '') IS NULL
       AND person.neon_auth_user_id::text = auth.user_id() AND EXISTS (
        ...
        person.auth0_user_id = NULLIF(current_setting('request.financing.user_id', TRUE), '') AND person.auth0_account_active
...
AS $$
DECLARE
	auth_user_id text := COALESCE(NULLIF(current_setting('request.financing.user_id', TRUE), ''), auth.user_id());
	actor_id text;
	actor_email text;
	row_json jsonb;
	entity_id text;
