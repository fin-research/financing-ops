...
<section class="detail-card source-card"><header><Database size={18} /><h2>完整字段</h2></header><div class="source-grid">
...
<style>
	...
	dl{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));margin:0}dl>div{padding:.75rem 1rem;border-bottom:1px solid var(--line)}dt{font-size:.75rem;color:var(--muted)}dd{margin:.25rem 0 0;font-weight:650;font-variant-numeric:tabular-nums}.cashflows{display:grid}.cashflows>div{display:grid;grid-template-columns:6.5rem 4rem minmax(0,1fr);gap:.75rem;padding:.75rem 1rem;border-bottom:1px solid var(--line)}.cashflows span{font-variant-numeric:tabular-nums}.cashflows p{padding:1rem;color:var(--muted)}
